...
class LinkedInOAuth2 extends OAuth2 {
	...
	public function getProfilByToken()
	{
		$fields = array(
			'current-status',
			'id',
	...
	public function shareStatus($content=array()){
		...
		if (is_array($content) AND count($content) > 0) {
			...
			$json['content'] = array();
			if (isset($content['title'])) $json['content']['title'] = $content['title'];
			if (isset($content['submitted-url'])) $json['content']['submitted-url'] = $content['submitted-url'];
			if (isset($content['submitted-image-url'])) $json['content']['submitted-image-url'] = $content['submitted-image-url'];
			if (isset($content['description'])) $json['content']['description'] = $content['description'];
	...
	public function postToGroup($group_id,$title,$message,$content=array()){
		$this->access_token = ( isset(get_setting()['linkedin_access_token']) ? get_setting()['linkedin_access_token'] : '' );
