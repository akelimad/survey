			...
			<ul id="menu_site_carriere" style="padding: 1px;">
				<?php if (read_session('id_type_role') == 1) : ?>
					...
					<li <?= (isset($_GET['id']) && $_GET['id']==0) ? 'class="menufo-active"' : ''; ?>>
						<a href="<?= site_url('backend/module/candidatures/candidature/list/0'); ?>"><i class="fa fa-spinner"></i>&nbsp;<?php trans_e("Nouvelles candidatures"); ?><span class="badge badge-error pull-right"><?= Candidature::countByStatus(0); ?></span></a>
					</li>
					<li <?= (!isset($_GET['id']) && isset($_GET['action']) && $_GET['action'] == 'index') ? 'class="menufo-active"' : ''; ?>>
						<a href="<?= site_url('backend/module/candidatures/candidature'); ?>"><i class="fa fa-list-ol"></i>&nbsp;<?php trans_e("Candidatures en cours"); ?><span class="badge badge-info pull-right"><?= Candidature::countPending(); ?></span></a>
					</li>
				<?php endif; ?>
				...
				<li <?= (App\Route::getRoute() == 'backend/candidatures/spontanees') ? 'class="menufo-active"' : ''; ?>>
					<a href="<?= site_url('backend/candidatures/spontanees'); ?>"><i class="fa fa-handshake-o"></i>&nbsp;<?php trans_e("Candidatures spontanées"); ?><span class="badge pull-right"><?= Candidature::countSpontanees(); ?></span></a>
				</li>
				<?php endif; ?>
				...
				<li <?= (App\Route::getRoute() == 'backend/candidatures/stage') ? 'class="menufo-active"' : ''; ?>>
					<a href="<?= site_url('backend/candidatures/stage'); ?>"><i class="fa fa-graduation-cap"></i>&nbsp;<?php trans_e("Candidatures pour stage"); ?><span class="badge pull-right"><?= Candidature::countStage(); ?></span></a>
				</li>
				<?php endif; ?>
			</ul>
		</div>
