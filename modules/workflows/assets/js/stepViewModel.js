...
* Step view model
*/
function StepViewModel(parent, isStart, id, name, contactType, depId, description, template, top, left) {
    var self = this;
    ...
    */
	...
	/**
    * Computed KnockOut function, returns user type info from depIds object depending on self.depId() value
    */
    self.user = ko.computed(function () {
		if(self.id() && self.depId()) {
			var depId = self.depId(),
				index = self.parent.depIds.mappedIndexOf({ Id: depId }),
				label = self.parent.depIds()[index].name();
    ...
    */
    ...
    //@@ self.isStart(isStart).id(id).name(name).contactType(contactType).depId(depId).description(description).template(template).top(top).left(left);
    self.id(id).depId(depId).description(description).top(top).left(left);
