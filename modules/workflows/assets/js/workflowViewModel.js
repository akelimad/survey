...
function WorkflowViewModel() {
	...
	self.depIds = ko.mapping.fromJS(demoData.depIds, dataMappingOptions);
    ...
    */
    ...
    self.addNewStep = function (contactType, top, left) {
		...
		var id = self.getNewId(),
			...
			//StepViewModel -> parent, isStart, id, name, contactType, depId, description, template, top, left
        	//@@ newStep = new StepViewModel(self, isStart, id, 'New Step '+id, contactType, 1, '', null, top, left);
        	newStep = new StepViewModel(self, isStart, id, 'Choisissez un utilisateur ', contactType, 1, '', null, top, left);
	...
	self.saveData = function() {
		...
		for (var i = 0; i < steps.length; i++) {
			...
			data.steps.push({isStart: steps[i].isStart(), Id: steps[i].id(), name: steps[i].name(), contactType: steps[i].contactType(), depId: steps[i].depId(), description: steps[i].description(), template: steps[i].template(), top: top, left: left});
        }
		for (var i = 0; i < results.length; i++) {
			...
			data.results.push({Id: results[i].id(), sourceId: results[i].sourceId(), targetId: results[i].targetId(), sourceEndpoint: results[i].sourceEndpoint(), targetEndpoint: results[i].targetEndpoint()});
        }
		...
		if (!Modernizr.localstorage) { 
			...
			if(current) {
				...
				self.saveDataToDB(data.name, current, JsonString)
				...
				bootbox.prompt("Give your workflow a name:", function(name) {                						   
					if(name) {
						...
						self.saveDataToDB(name, key, JsonString)
	...
	self.saveDataToDB = function(name, reference, value) {
		ajax_handler({
			data: {
	...
	self.deleteAllWorkflows = function() {
		...
		function verify() {	
			bootbox.dialog({
				message: "Êtes-vous sûr de vouloir supprimer tous les workflows sauvegardés?",
				buttons: {
					main: {
						label: "Annuler",
    ...
    */
	function getData(data) {
		...
		for (var i = 0; i < steps.length; i++) {
			var stepViewModel = new StepViewModel(self, steps[i].isStart, steps[i].Id, steps[i].name, steps[i].contactType, steps[i].depId, steps[i].description, steps[i].template, steps[i].top, steps[i].left);
    ...
    */
	function unsavedWarning(func) {
		bootbox.dialog({
			message: "Vous avez des modifications non enregistrées dans ce workflow. Si vous procédiez, ces modifications seront perdues. Êtes-vous sûr de vouloir continuer sans sauvegarder?",
			title: "Warning!",
			buttons: {
				main: {
					label: "Cancel",
