<?php
 ...
 // ///////////////////////////Ajout des variables get pour la conservation du filtrage lors de la pagination//////////
        ...
        $req = (isset ( $requete ) && ! empty ( $requete )) ? mysql_query ( "select COUNT(*) AS nb_offres FROM offre where offre.date_expiration >= '$today' AND ( " . $requete . " ) And status = 'En cours' AND id_tpost=4 " ) : mysql_query ( "select COUNT(*) AS nb_offres FROM offre where offre.date_expiration >= '$today' AND id_tpost=4  " );
        ...
        $select = isset ( $requete ) && ! empty ( $requete ) ? mysql_query ( "select * from offre  
                  ...
                  where ( " . $requete . " )  AND id_tpost=4  ORDER BY " . $order . "   
