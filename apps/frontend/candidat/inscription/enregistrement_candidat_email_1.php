...
$select = mysql_query("select * from root_email_auto where ref='b' ");
            ...
            // Génère : message
                ...
                $replace   = array($nom_s, $email1, $mdp1);
...
$sql_mail= ' INSERT into corespondances 
Values ("","'.safe($obj_emp).'","'.safe($nom_s).'","'.safe($d).'","'.safe($type_email).'","'.safe($type_c).'","'.safe($message_employeur).'") ';
