...
$dossier = '74b87337454200d4d33f80c4663dc5e5/';
...
//Début des vérifications de sécurité...
		...
		$j=1;
...

			{
				...
				$erreur[$k][0]="Le CV ".$fichier." n'est pas uploadé à cause des problèmrd suivants";
...

			{
			 ...
			 $erreur[$k][$j] = 'La taille du CV d&eacute;passe les :'.$taille_maxi_ko." Ko" ;
			 ...
			 $fich = preg_replace('/([^.a-z0-9]+)/i', '-', $fich);
				...
				if(move_uploaded_file($_FILES['upload']['tmp_name'][$i], $dossier . $fich)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...

				 {
				  ...
				  if(mysql_query("INSERT into cv values('','".safe($fichier)."','".safe($fich)."','".safe($candidats_id)."','0','1')"))

				  {

				  

				  $msg[$k]="Le CV ".$fichier." est upload&eacute; avec succ&egrave;s";
				  ...
				  $erreur[$k][$j] = "une erreur s'est produite lor de l'enregistrement, de votre CV" ;
	...
	$_SESSION['erreur']=isset($erreur) ? $erreur : ""; 
