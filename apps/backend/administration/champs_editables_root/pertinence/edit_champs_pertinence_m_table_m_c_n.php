...
$min_p_a_req = (isset($_POST['min_p_a_req']))? intval($_POST['min_p_a_req']) : 0;
...
*/
		...
		$prm_pertinence = mysql_query("SELECT prm_titre,prm_expe,prm_local,prm_tpost,prm_mobil,prm_n_mobil,prm_t_mobil,prm_fonc,prm_nfor FROM `prm_pertinence` WHERE   ref_p = '".$_POST['ref_p']."' limit 0,1 ");
		$r_prm_pertinence = mysql_fetch_assoc($prm_pertinence); 
		 
					
	//	  echo  "<br>--------------------<br>r_prm_pertinence<br>--------------------<br>" ;
		 $prm_titre			=			$r_prm_pertinence['prm_titre']		;
		 $prm_expe			=			$r_prm_pertinence['prm_expe']		;
		 $prm_local			=			$r_prm_pertinence['prm_local']		;	
		 $prm_tpost			=			$r_prm_pertinence['prm_tpost']		;
		 $prm_mobil			=			$r_prm_pertinence['prm_mobil']		;
		 $prm_n_mobil		=			$r_prm_pertinence['prm_n_mobil']	;	
		 $prm_t_mobil		=			$r_prm_pertinence['prm_t_mobil']	;
		 $prm_fonc			=			$r_prm_pertinence['prm_fonc']		;
		 $prm_nfor  		=			$r_prm_pertinence['prm_nfor']		;           
		 
 
 
 
 
$sql_tbl_offre_p =  "SELECT id_offre,Name,id_expe,id_localisation,id_tpost,mobilite,niveau_mobilite,taux_mobilite,id_fonc,id_nfor FROM offre  " ;
 /*limit 0,10
  echo  "<br>--------------------<br>"; 
...
$tbl_offre_p = mysql_query($sql_tbl_offre_p);
 ...
 $sql_tbl_candidats_p = "SELECT candidats_id FROM candidats 
    where  candidats_id  NOT IN ( SELECT candidats_id FROM candidature 
        WHERE id_offre=".$r_offre['id_offre']." and candidats_id IS NOT NULL)  ";
/* limit 0,10
 echo  "<br>--------------------<br>"; 
...
while($r_candidats_req = mysql_fetch_assoc( $candidat_pertinence )){
 ...
 $t_prm_activ = $prm_titre + $prm_expe + $prm_local + $prm_tpost + $prm_mobil + $prm_n_mobil + $prm_t_mobil + $prm_fonc	+ $prm_nfor ;
	...
	//	  echo  "<br>--------------------<br> if candidats <br>--------------------<br>" ; 	  
		 ...
		 if ( $prm_n_mobil==1  and  $o_n_mobil== $c_n_mobil and  $o_mobil=="non" )   {
		   $t_r_match_n_mobil  = 1;  $p__match_n_mobil  = 100;
		} 
		 
		 if ( $prm_t_mobil==1  and  $o_t_mobil== $c_t_mobil and  $o_mobil=="non" )   {
		   $t_r_match_t_mobil  = 1; $p__match_t_mobil  = 100;
		} 
		 
		 if ( $prm_t_mobil==1  and  $o_fonc== $c_fonc)   {
		   $t_r_match_fonc  = 1;$p__match_fonc  = 100;
		} 
		 
		 if ( $prm_fonc==1  and  $o_nfor== $c_nfor)   {
	...
	//	  echo  "<br>--------------------<br> resulta <br>--------------------<br>" ; 
		 ...
		 if(!empty($t_prm_activ) and !empty($t_r_match)){ $r_match_result=($t_r_match/$t_prm_activ); }
