...
if($status=='Présélection') $status='Présélection';

if($status=='Entretien Métier') $status='Entretien Métier';

if($status=='Entretien Direction Métier') $status='Entretien Direction Métier';
...
$lien_confirmation='<a href="'.$site.'confirmation/?i='.$id_agenda.'"> <b>Confirmer</b></a> '; /*'<a href="'.$site.'confirmation_statu.php?id_agenda='.$id_agenda.'"> <b>Confirmer</b></a> ';*/
...
//=====Lecture et mise en forme de la pièce jointe.
 ...
 $fichier=file_get_contents('../../../apps/upload/upload_courrier/'.$p_joint);
...
$msg_pj= "Content-Type: application/msword; name=\"$p_joint\"\r\n".
            ...
            "$fichier";
                ...
                $var = array("{{nom_candidat}}", "{{date_postulation}}", "{{statu_candidature}}", "{{date_statu}}", "{{lieu_statu}}", "{{lien_confirmation}}", "{{titre_offre}}");  $replace   = array($nom_s,$date_postulation , $statu_candidature, $date_statu,$lieu_statu, "", $titre_offre);
...
$sql_mail= ' INSERT into corespondances Values ("","'. safe($obj_emp).'","'. safe($nom_s).'",
    ...
    "'. safe($d).'","'. safe($type_email).'","'. safe($obj_emp).'",

    "'.$message_employeur.'","'.$ref_filiale.'","") ';
                ...
                $mail_msg="<br>Un email sera envoy&eacute;  automatiquement au candidat ";
