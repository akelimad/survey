...
if(isset($_POST['id_candidature']) &&  !empty($_POST['dossier']))
{
			...
			$select  = mysql_query("SELECT * FROM campagne_offres WHERE candidats_id = '$id_candidature' AND id_compagne = '$dossier'  ");
			$count=0;
			while ($row = mysql_fetch_assoc($select)) {
				...
				if ($count == 0) 
						{	
						mysql_query("INSERT INTO campagne_offres VALUES ('$dossier','$id_candidature')");
