...
if(isset($_POST['id_list']) &&  !empty($_POST['dossier']))
{
			...
			$tab_id = explode(",", $id_list);
				...
				$count=0;
				$sql_s="SELECT * FROM dossier_candidat WHERE candidats_id = '".$tab_id[$i]."' AND id_dossier = '$dossier'  ";
				echo $sql_s.'<br>';
			...
			while ($row = mysql_fetch_assoc($select)) {
				...
				if ($count == 0) 
						{	
						$sql_i="INSERT INTO dossier_candidat VALUES ('".safe($dossier)."','".safe($tab_id[$i])."')";
						mysql_query($sql_i);
						echo $sql_i.'<br>';
