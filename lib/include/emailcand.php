...
if( isset($_GET['exp']) && isset($_GET['dest']))
{
		$msg = $_GET['message'];
		$expediteur = $_GET['exp'];
		...
		$subject = $_GET['sujet'];
		$replyto = $expediteur;
		$my_path = $_SERVER['DOCUMENT_ROOT']."/".$rep_path."assets/upload/cv/";
	
		/*
		$filename = $_GET['attach'];
			$filename2 = $_GET['attach2'];
		$path = dirname(__FILE__)."/../../assets/upload/cv/".$filename."";
		$path2 = $_SERVER['DOCUMENT_ROOT']."/".$rep_path."assets/upload/lmotivation/".$filename2."";
...
$repertoire=dirname(__FILE__)."/../../assets/upload/cv/";
...
if( $file1== $fichier1 )
{
$fichier=dirname(__FILE__)."/../../assets/upload/cv/".$file;
$path=$fichier;
}
